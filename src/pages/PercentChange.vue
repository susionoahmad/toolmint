<template>
  <ToolLayout 
    ...
    description="Calculate the percentage increase or decrease between two numbers instantly."
  >
    <template #inputs>
      <InputCard title="Value Details">
        <template #icon><RefreshIcon class="w-5 h-5 text-slate-400" /></template>
        
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Old Value (Starting)</label>
            <input 
              type="number" 
              v-model.number="store.percentChangeState.oldValue" 
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">New Value (Final)</label>
            <input 
              type="number" 
              v-model.number="store.percentChangeState.newValue" 
              class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
              placeholder="e.g. 150"
              @input="handleInput"
            />
        ...
        <div class="mt-8 border-t border-slate-100 pt-6">
          ...
          >
            Calculate Change (Instant Result)
          </button>
        </div>
      </InputCard>
    </template>
    ...
    <template #results>
      <ResultCard title="Analysis Result">
        <template #icon><TrendingUpIcon class="w-5 h-5 text-blue-200" /></template>
        ...
        <div v-else class="space-y-6 text-center">
          <div class="bg-blue-800/40 p-6 rounded-2xl border border-blue-500/20 relative overflow-hidden transition-all duration-500">
            <div :class="[showGate ? 'blur-sm opacity-60 select-none pointer-events-none' : 'opacity-100', 'transition-all duration-[600ms]']">
              <div class="text-blue-100 text-sm font-medium uppercase tracking-wider mb-1">Percentage Change</div>
              <div :class="[isIncrease ? 'text-emerald-400' : 'text-red-400', 'text-6xl font-extrabold mb-2']">
                {{ isIncrease ? '+' : '' }}{{ changeValue.toFixed(2) }}%
              </div>
              <p class="text-blue-200 text-sm">
                {{ isIncrease ? 'Increase' : 'Decrease' }} from {{ store.percentChangeState.oldValue }} to {{ store.percentChangeState.newValue }}
              </p>
              
              <div class="grid grid-cols-2 gap-4 mt-6 border-t border-blue-500/30 pt-4 text-left">
                <div>
                  <div class="text-blue-300 text-xs uppercase tracking-wide">Difference</div>
                  <div class="text-lg font-bold text-white">{{ (store.percentChangeState.newValue - store.percentChangeState.oldValue).toFixed(2) }}</div>
                </div>
                <div>
                  <div class="text-blue-300 text-xs uppercase tracking-wide">Ratio</div>
                  <div class="text-lg font-bold text-blue-100">{{ (store.percentChangeState.newValue / (store.percentChangeState.oldValue || 1)).toFixed(2) }}x</div>
                </div>
              </div>
            </div>
            ...
            <div v-if="showGate" class="absolute inset-0 flex flex-col items-center justify-center z-10 pointers-events-none">
              ...
              <span class="text-white font-bold drop-shadow-lg text-sm bg-blue-900/60 px-3 py-1 rounded-lg backdrop-blur-sm">Unlock Detailed Analysis</span>
            </div>
          </div>
          
          <DownloadGate 
            v-if="showGate"
            action-text="Unlock Detailed Analysis"
...
import { RefreshCcw as RefreshIcon, TrendingUp as TrendingUpIcon } from 'lucide-vue-next';

onMounted(() => {
  setSEO('Percentage Change Calculator - Growth Tracker', 'Instantly calculate the percentage increase or decrease between two values.');
});
...
const isValid = computed(() => store.percentChangeState.oldValue !== undefined);
...
const changeValue = computed(() => {
  const oldVal = store.percentChangeState.oldValue || 1;
  const newVal = store.percentChangeState.newValue || 0;
  return ((newVal - oldVal) / Math.abs(oldVal)) * 100;
});

const isIncrease = computed(() => changeValue.value >= 0);
