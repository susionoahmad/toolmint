<template>
  <ToolLayout 
    ...
    description="Calculate exact PayPal fees and how much you need to charge to receive a specific amount."
  >
    <template #inputs>
      <InputCard title="Transaction Details">
        <template #icon><WalletIcon class="w-5 h-5 text-slate-400" /></template>
        ...
        <div class="space-y-4">
          <div>
            ...
            <input 
              type="number" 
              v-model.number="store.paypalState.amount" 
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input 
                type="number" 
                v-model.number="store.paypalState.rate" 
                class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                placeholder="4.4"
                @input="handleInput"
              />
            ...
            <div>
              ...
              <input 
                type="number" 
                v-model.number="store.paypalState.fixed" 
                class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                placeholder="0.30"
                @input="handleInput"
              />
        ...
        <div v-else class="space-y-6">
          ...
          <div class="bg-blue-800/40 p-6 rounded-2xl border border-blue-500/20 relative overflow-hidden transition-all duration-500">
            <div :class="[showGate ? 'blur-sm opacity-60 select-none pointer-events-none' : 'opacity-100', 'transition-all duration-[600ms]']">
              ...
              <div class="space-y-6">
                <!-- If I receive X -->
                <div class="pb-6 border-b border-blue-500/20">
                  <div class="text-blue-300 text-xs uppercase font-semibold mb-2">If you receive {{ formatUSD(store.paypalState.amount) }}</div>
                  <div class="flex justify-between items-end">
                    <div>
                      <div class="text-blue-100 text-sm">You Get (Net)</div>
                      <div class="text-3xl font-extrabold text-white">{{ formatUSD(receiveAmounts.net) }}</div>
                    </div>
                    <div class="text-right">
                      <div class="text-blue-400 text-xs">Total Fee</div>
                      <div class="text-lg font-bold text-red-400">-{{ formatUSD(receiveAmounts.fee) }}</div>
                    </div>
                  </div>
                </div>
                ...
                <!-- To receive X -->
                <div>
                  <div class="text-blue-300 text-xs uppercase font-semibold mb-2">To receive exactly {{ formatUSD(store.paypalState.amount) }}</div>
                  <div class="flex justify-between items-end">
                    <div>
                      <div class="text-blue-100 text-sm">You should ask for</div>
                      <div class="text-3xl font-extrabold text-emerald-400">{{ formatUSD(askAmounts.total) }}</div>
                    </div>
                    <div class="text-right">
                      <div class="text-blue-400 text-xs">Total Fee</div>
                      <div class="text-lg font-bold text-red-400">-{{ formatUSD(askAmounts.fee) }}</div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div v-if="showGate" class="absolute inset-0 flex flex-col items-center justify-center z-10 pointers-events-none">
              ...
              <span class="text-white font-bold drop-shadow-lg text-sm bg-blue-900/60 px-3 py-1 rounded-lg backdrop-blur-sm">Unlock Net Value</span>
            </div>
          </div>
          
          <DownloadGate 
            v-if="showGate"
            action-text="Unlock Net Value"
...
import { Wallet as WalletIcon, Calculator as CalculatorIcon } from 'lucide-vue-next';

onMounted(() => {
  setSEO('PayPal Fee Calculator - Calculate Your Net Profit', 'Instantly find out how much PayPal fees you will pay and how much to charge to receive your desired net amount.');
});
...
const isValid = computed(() => !!store.paypalState.amount && store.paypalState.rate >= 0);
...
const receiveAmounts = computed(() => {
  const am = store.paypalState.amount || 0;
  const rate = (store.paypalState.rate || 0) / 100;
  const fixed = store.paypalState.fixed || 0;
  const fee = (am * rate) + fixed;
  return { fee, net: am - fee };
});

const askAmounts = computed(() => {
  const am = store.paypalState.amount || 0;
  const rate = (store.paypalState.rate || 0) / 100;
  const fixed = store.paypalState.fixed || 0;
  // Formula: Total = (Net + Fixed) / (1 - Rate)
  const total = (am + fixed) / (1 - rate);
  return { total, fee: total - am };
});
